...
import { FORUM_POST_TYPES } from "@/lib/platform"
...
export async function POST(req: Request) {
  ...
  if (!title || !content) {
    return NextResponse.json({ success: false, message: "Titulo e conteudo sao obrigatorios." }, { status: 400 })
  }
  ...
  if (postType === "EVENTO" && !isStaffRole(session.user.role)) {
    return NextResponse.json(
      { success: false, message: "Somente a equipe K9 pode publicar eventos no forum." },
      { status: 403 },
    )
  }

  if (title.length < 6 || title.length > 120) {
    return NextResponse.json({ success: false, message: "Titulo deve ter entre 6 e 120 caracteres." }, { status: 400 })
  }

  if (content.length < 20 || content.length > 2000) {
    return NextResponse.json({ success: false, message: "Conteudo deve ter entre 20 e 2000 caracteres." }, { status: 400 })
  }
  ...
  if (channelId) {
    ...
    if (!channel?.id) {
      return NextResponse.json({ success: false, message: "Canal invalido para publicacao." }, { status: 400 })
    }

    const canPostInChannel =
      channel.isPublic || channel.ownerId === session.user.id || isAdminRole(session.user.role) || channel.subscriptions.length > 0
